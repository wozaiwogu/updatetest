...
CString MD5::GetMd5()
{
	MD5 m;
	m.init();
	std::string md5 = m._GetMd5();
	CString cstrMd5(md5.c_str());
	m.Finalize();
	return cstrMd5;
}
...
string MD5::_GetMd5()
{
	...
	srand(tmDst);
	int temp = rand() % 1000;
	std::string str = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxy" + to_string(temp);
	const char *md5Char = str.c_str();
	int len = strlen(md5Char);
	ComputMd5(md5Char, len);
	if (!isDone_)
	{
		cout << "Error: computation is not finished" << endl;
		exit(0);
